...
#             nn.BatchNorm2d(embed_dim)
#         )
#     # torch.Size([1, 64, 56, 56]) -> torch.Size([1, 56, 56, 64])
#     def forward(self, x):
#         B, C, H, W = x.shape
...
#         self.embed_dim = embed_dims[0]  # 64
#         self.patch_norm = patch_norm   # True
#         self.num_features = embed_dims[-1]  # 512
#         self.mlp_ratios = mlp_ratios  # 4，4，3，3

#         # torch.Size([1, 3, 224, 224]) ->  torch.Size([1, 56, 56, 64])
#         # split image into non-overlapping patches   3，64，torch.nn.modules.normalization.LayerNorm'
#         self.patch_embed = PatchEmbed(in_chans=in_chans, embed_dim=embed_dims[0],
#             norm_layer=norm_layer if self.patch_norm else None)


#         # stochastic depth [3, 4, 18, 4]
...
#             layer = BasicLayer(
#                 embed_dim=embed_dims[i_layer],  # [64, 128, 256, 512],out是下一层的input，最后是None
#                 out_dim=embed_dims[i_layer+1] if (i_layer < self.num_layers - 1) else None,
#                 depth=depths[i_layer], # [3, 4, 18, 4]
#                 num_heads=num_heads[i_layer],  # [4, 4, 8, 16]
#                 init_value=init_values[i_layer], # [2, 2, 2, 2]
#                 heads_range=heads_ranges[i_layer], # [4, 4, 6, 6]
...
#         # apply 方法，递归地遍历模型的所有子模块,并对每个子模块应用指定的函数。
#         self.apply(self._init_weights)

#     def _init_weights(self, m):
#         # 如果子模块 m 是一个线性层（nn.Linear），则对其权重进行截断正态分布初始化
#         #（truncated normal initialization），标准差为 0.02。如果该线性层存在偏置参数，
#         # 则将偏置参数初始化为 0。
#         # 如果子模块 m 是一个 LayerNorm 层（nn.LayerNorm），则对其偏置参数初始化为 0，
#         # 对其标准化参数（即缩放参数）初始化为 1。
...
@jit.script
def swish_script(x: torch.Tensor) -> torch.Tensor:
    return x * torch.sigmoid(x)

class MemoryEfficientSwish(nn.Module):
    def forward(self, x):
        return swish_script(x)
...
class RetBlock(nn.Module):
    ...
    def __init__(self, retention: str, embed_dim: int, num_heads: int, ffn_dim: int, drop_path=0., layerscale=False, layer_init_values=1e-5):
        ...
        self.final_layer_norm = nn.LayerNorm(self.embed_dim, eps=1e-6)
        self.ffn = FeedForwardNetwork(embed_dim, ffn_dim)
        self.pos = DWConv2d(embed_dim, 3, 1, 1)  # 卷积操作，捕捉空间关系
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            if self.use_checkpoint:
                tmp_blk = partial(blk, incremental_state=None, chunkwise_recurrent=self.chunkwise_recurrent, retention_rel_pos=rel_pos)
                x = checkpoint.checkpoint(tmp_blk, x)  # 这里进行了推理
            else:
                x = blk(x, incremental_state=None, chunkwise_recurrent=self.chunkwise_recurrent, retention_rel_pos=rel_pos)
